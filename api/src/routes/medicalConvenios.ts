...
export const medicalConveniosRouter = Router();

/**
 * Se o Prisma Client não foi gerado após adicionar os models novos,
 * os delegates (ex.: prisma.medicalConvenioInvoice) ficam undefined e
 * estoura TypeError em runtime.
 */
function assertMedicalConveniosModels() {
  const p: any = prisma as any;
  if (!p?.medicalConvenioInvoice || !p?.medicalConvenioUsage) {
    throw new Error(
      "Prisma Client desatualizado para Convênios Médicos. Rode `npx prisma generate` (na pasta api) e reinicie a API."
    );
  }
}

/**
 * Provider slugs (URL) -> enum do Prisma
 */
const PROVIDER_MAP = {
  "laboratorio-santa-cruz": "LABORATORIO_SANTA_CRUZ",
  "centro-diagnostico-capao-raso": "CENTRO_DIAGNOSTICO_CAPAO_RASO",
  "policlinica-capao-raso": "POLICLINICA_CAPAO_RASO",
  "policlinica-mansur": "POLICLINICA_MANSUR",
} as const;

type ProviderSlug = keyof typeof PROVIDER_MAP;
type ProviderEnum = (typeof PROVIDER_MAP)[ProviderSlug];

function getProvider(slugRaw: string): { slug: ProviderSlug; provider: ProviderEnum } {
  const slug = String(slugRaw || "").trim() as ProviderSlug;
  const provider = (PROVIDER_MAP as any)[slug] as ProviderEnum | undefined;
  if (!provider) {
...
function split(kind: "PERSONAL" | "WORK_ACCIDENT", amountTotal: unknown) {
...
// GET /medical-convenios/:provider/invoices/by-month?month=YYYY-MM
medicalConveniosRouter.get("/:provider/invoices/by-month", async (req, res) => {
  try {
    const { provider } = getProvider(String(req.params.provider));
    assertMedicalConveniosModels();
    const month = String(req.query.month ?? "").trim();
    if (!month) return res.status(400).json({ message: "month é obrigatório (YYYY-MM)" });

    const competence = parseMonthToDate(month);

    const invoice = await prisma.medicalConvenioInvoice.findFirst({
      where: { provider, competence },
      select: {
        id: true,
        competence: true,
        invoiceNumber: true,
    ...
    if (String(msg).includes("Provider inválido")) return res.status(400).json({ message: msg });
		if (String(msg).includes("Prisma Client desatualizado")) return res.status(500).json({ message: msg });
...
// POST /medical-convenios/:provider/invoices
medicalConveniosRouter.post("/:provider/invoices", async (req, res) => {
  try {
    const { provider } = getProvider(String(req.params.provider));
    assertMedicalConveniosModels();
    ...
    const already = await prisma.medicalConvenioInvoice.findFirst({
      where: { provider, competence },
      select: { id: true },
    });
    ...
    try {
      const created = await prisma.medicalConvenioInvoice.create({
        data: {
          provider,
          competence,
          invoiceNumber,
      ...
      // concorrência (mesmo mês + provider)
      if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2002") {
        const existing = await prisma.medicalConvenioInvoice.findFirst({
          where: { provider, competence },
          select: { id: true },
        });
...
// PATCH /medical-convenios/:provider/invoices/:id  (salvar cabeçalho)
medicalConveniosRouter.patch("/:provider/invoices/:id", async (req, res) => {
  try {
    const { provider } = getProvider(String(req.params.provider));
    assertMedicalConveniosModels();
    ...
    const invoice = await prisma.medicalConvenioInvoice.findFirst({ where: { id, provider }, select: { status: true } });
    ...
    await prisma.medicalConvenioInvoice.update({
      where: { id },
      data: {
...
// GET /medical-convenios/:provider/invoices/:id  (detalhes + lista de funcionários do mês)
medicalConveniosRouter.get("/:provider/invoices/:id", async (req, res) => {
  try {
    const { provider } = getProvider(String(req.params.provider));
    assertMedicalConveniosModels();
    const id = Number(req.params.id);
    ...
    const msg = err?.message || "Erro interno ao carregar invoice.";
    if (String(msg).includes("Provider inválido")) return res.status(400).json({ message: msg });
		if (String(msg).includes("Prisma Client desatualizado")) return res.status(500).json({ message: msg });
    console.error(err);
...
// PATCH /medical-convenios/:provider/invoices/:id/usages/:employeeId
medicalConveniosRouter.patch("/:provider/invoices/:id/usages/:employeeId", async (req, res) => {
  try {
    const { provider } = getProvider(String(req.params.provider));
    assertMedicalConveniosModels();
    ...
    await prisma.$transaction(async (tx) => {
      await tx.medicalConvenioUsage.deleteMany({ where: { invoiceId, employeeId } });
      if (cleaned.length) {
        await tx.medicalConvenioUsage.createMany({
          data: cleaned.map((u) => {
            const sp = split(u.kind, u.amountTotal);
            return {
              invoiceId,
              employeeId,
              kind: u.kind,
              amountTotal: sp.amountTotal,
              amountEmployee: sp.amountEmployee,
              amountCompany: sp.amountCompany,
...
// POST /medical-convenios/:provider/invoices/:id/close
medicalConveniosRouter.post("/:provider/invoices/:id/close", async (req, res) => {
  try {
    const { provider } = getProvider(String(req.params.provider));
    assertMedicalConveniosModels();
    const id = Number(req.params.id);
...
// POST /medical-convenios/:provider/invoices/:id/reopen
medicalConveniosRouter.post("/:provider/invoices/:id/reopen", async (req, res) => {
  try {
    const { provider } = getProvider(String(req.params.provider));
    assertMedicalConveniosModels();
    const id = Number(req.params.id);
    ...
    await prisma.medicalConvenioInvoice.update({ where: { id }, data: { status: "DRAFT", closedAt: null } });
    return res.json({ ok: true, status: "DRAFT" });
  } catch (err: any) {
    const msg = err?.message || "Erro interno ao reabrir mês.";
    if (String(msg).includes("Provider inválido")) return res.status(400).json({ message: msg });
    if (String(msg).includes("Prisma Client desatualizado")) return res.status(500).json({ message: msg });
    console.error(err);
